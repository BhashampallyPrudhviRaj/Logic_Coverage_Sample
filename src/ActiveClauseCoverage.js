...
const ActiveClauseCoverage = () => {
  ...
  const generateTruthTable = () => {
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      const result = eval(predicate.replace(/[a-zA-Z]/g, match => row[match]));
      row['Result'] = result;
      table.push(row);
    }
  ...
  return (
    <div>
      ...
      <button onClick={generateClausePairs}>Generate Clause Pairs</button>
...
class ActiveClauseCoverage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      predicate: '',
  ...
  generateTruthTable = () => {
    ...
    this.setState({ truthTable: table });
  };
  ...
  evaluateExpression = (expr, row) => {
    ...
    variables.forEach(variable => {
      ...
      expressionCopy = expressionCopy.replace(regex, value);
    });
  ...
  generateClausePairs = () => {
    const { variables, truthTable } = this.state;
    const clausePairs = [];
    ...
    variables.forEach((variable, k) => {
      ...
      truthTable.forEach((row, i) => {
        const tVar = this.evaluateExpression(this.state.predicate, {
          ...row,
          [variable]: true,
        });
        const fVar = this.evaluateExpression(this.state.predicate, {
          ...row,
          [variable]: false,
        });
      ...
      const pairs = [];
      trueList.forEach(tIndex => {
        falseList.forEach(fIndex => {
          pairs.push([tIndex, fIndex]);
        });
      });
    ...
    this.setState({ clausePairs });
  };

  render() {
    const { predicate, variables, truthTable, clausePairs } = this.state;
    ...
    return (
      <div>
        ...
        <button onClick={this.generateClausePairs}>Generate Clause Pairs</button>
  ...
  return (
    <div className="active-clause-coverage">
      ...
      <input
        ...
        placeholder="Enter predicate equation"
      />
      {!truthTableGenerated && <button onClick={generateTruthTable}>Generate Truth Table</button>}
      {truthTableGenerated && !clausePairsGenerated && <button onClick={generateClausePairs}>Generate Clause Pairs</button>}
      {clausePairsGenerated && <button onClick={() => {setTruthTableGenerated(false); setClausePairsGenerated(false)}}>Generate Truth Table</button>}
...
const ActiveClauseCoverage = () => {
  ...
  const handleChange = (event) => {
    ...
    if (!value.trim()) {
      ...
      const invalidChars = value.match(/[^a-zA-Z0-9()&|!^= ]|(?<![a-zA-Z0-9])[0-9]+(?![a-zA-Z])/g);
      if (invalidChars) {
